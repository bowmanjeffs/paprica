...
if len(sys.argv) > 2:               
    ...
    edpl = 'test.bacteria.bacteria.combined_16S.bacteria.tax.clean.align.edpl.csv'
    query = 'test.bacteria.bacteria.combined_16S.bacteria.tax.clean.align.csv'
    name = 'test.bacteria'
...
## Read in the edpl csv file generated by paprica_place-it and
## combine with query_csv.

edpl_csv = pd.read_csv(cwd + edpl, header = None, names = ['edpl'])
query_csv.loc[edpl_csv.index, 'edpl'] = edpl_csv.edpl
...
if unique != False:
    ...
    unique_csv = pd.read_csv(cwd + unique, header = 0, index_col = 0)
